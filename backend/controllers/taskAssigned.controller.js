...
exports.createTask = async (req, res) => {
    try {
        ...
        if (dueDate > new Date()) return res.status(400).json({ message: 'Due Date must be past date.' });

        // Ensure the assigned employee exists
        const employeeExists = await Employee.findById(assignedTo).populate("user");
        if (!employeeExists) return res.status(404).json({ message: 'Assigned employee not found' });
        ...
        await sendEmail(employeeExists.user.email, 'New Task Assigned', `You have been assigned a new task: ${title} By ${req.user.name}`);
...
exports.updateTask = async (req, res) => {
    try {
        ...
        if (dueDate && dueDate > new Date()) return res.status(400).json({ message: 'Due Date must be past date.' });
        ...
        const task = await TaskAssigned.findById(taskId).populate({
            path: 'assignedTo',
            populate: { path: 'user' }
        ...
        await sendEmail(oldTask.assignedTo.user.email, 'Task Updated', `Your assigned task has been updated. \n\nPrevious details: ${JSON.stringify(oldTask)} \n\nNew Task details: ${JSON.stringify(task)}`);
...
exports.addConclusion = async (req, res) => {
    try {
        ...
        if (task.assignedTo.toString() !== userId.toString() && task.assignedBy.toString() !== userId.toString() && req.user.role !== "CompanyAdmin") {
            return res.status(403).json({ message: 'Permission denied' });
        }
        ...
        // Send email notification
        await sendEmail(task.assignedBy.email, 'Task Conclusion Added', `A conclusion has been added to the task. \n\nConclusion is: "${conclusion}"`);
...
exports.getMyTasks = async (req, res) => {
    try {
        ...
        const tasks = await TaskAssigned.find({ assignedTo: employee._id })
