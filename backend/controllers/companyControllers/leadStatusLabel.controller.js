...
exports.addLeadStatusLabel = async (req, res) => {
    try {
        ...
        const newLeadStatusLabel = await LeadStatusLabel.create({ name, company: req.user.company });
        return res.status(201).json({ message: "Lead Status Label added successfully", data: newLeadStatusLabel });
...
exports.getAllLeadStatusLabel = async (req, res) => {
    try {
        const leadStatusLabel = await LeadStatusLabel.find({company: req.user.company}).populate("company", "name");

        return res.status(200).json({ message: "leadStatusLabel fetched successfully", data: leadStatusLabel });
